...
const ranges: Partial<Record<keyof Settings, [number, number, boolean]>> = {
  steps: [3, 20, true],
  anchorIndex: [0, 19, true],
  neutralSteps: [10, 14, true],
  hueShift: [-60, 60, false],
  tintStrength: [0, 0.08, false],
  normalText: [1, 21, false],
  nonText: [1, 21, false],
};
  ...
  updateUrlParams(
    Object.fromEntries(
      (scope === 'compare' ? comparisonKeys : keys).map((key) => [
        `${scope}.${key}`,
        settings[key] === DEFAULTS[key] ? null : settings[key],
      ]),
    ),
    'replace',
  );
