...
const modules = import.meta.glob<{
  default: ComponentType<{ components: typeof markdownComponents }>;
}>('../docs/articles/*.mdx', { eager: true });
const groups = [
...
export function Guide() {
  ...
  useEffect(() => {
    setExpanded((previous) =>
      previous.includes(current.group) ? previous : [...previous, current.group],
    );
  }, [current.group]);
  ...
  useEffect(() => {
    const sync = () => {
      setDocumentId(resolveId());
    };
    window.addEventListener('popstate', sync);
    return () => window.removeEventListener('popstate', sync);
  }, []);
  useEffect(() => {
    document.title = `${title} · OKRamp`;
  }, [title]);
  useEffect(() => {
  ...
  const matches = articles.filter((item) =>
    item.title.join(' ').toLocaleLowerCase().includes(search),
  );
  return (
    <div className="guide-docs-shell guide-chapters-shell">
      <aside className="guide-doc-nav" aria-label={t('指南章节', 'Guide chapters')}>
        ...
        <nav aria-label={t('文章导航', 'Article navigation')}>
          <Menu
            ...
            expanded={
              search
                ? groups
                    .filter(([id]) => matches.some((item) => item.group === id))
                    .map(([id]) => id)
                : expanded
            }
        ...
        {!matches.length && (
          <p className="guide-nav-empty" role="status">
      ...
      <article className="guide-document">
        ...
        <div className="guide-article-body" ref={body}>
          <Article components={markdownComponents} />
        </div>
        <footer className="guide-pagination" aria-label={t('相邻文章', 'Adjacent articles')}>
          {[articles[index - 1], articles[index + 1]].map((item, i) =>
            item ? (
              <a key={i} href={href(item.id)} onClick={(event) => select(event, item.id)}>
                <small>{i === 0 ? t('上一篇', 'Previous') : t('下一篇', 'Next')}</small>
                <span>{item.title[isZh ? 0 : 1]}</span>
              </a>
            ) : (
              <span key={i} />
            ),
        ...
        <nav>
          {headings.map((item) => (
            <a
              key={item.id}
              href={`#${item.id}`}
              className={`${item.level === 3 ? 'is-subheading ' : ''}${active === item.id ? 'is-active' : ''}`}
