...
if (pkg.name !== 'oklch-ramp' || pkg.private || !/^\d+\.\d+\.\d+$/.test(pkg.version)) {
  throw new Error('Expected a public stable oklch-ramp package.');
}
...
const body = `This PR updates the package version and changelog for npm publication.\n\nMerge after CI passes. Successful main CI will publish oklch-ramp@${version}; no GitHub Release is created.\n\n${notes}`;
const pr = existing
