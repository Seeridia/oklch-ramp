...
it('reports default neutral spacing as information with measured pairs', () => {
  ...
  const message = result.diagnostics.messages.find((item) => item.code === 'LOW_ADJACENT_DIFFERENCE');
  expect(message?.severity).toBe('info');
  expect(message?.details).toMatchObject({ scale: 'neutral', threshold: 0.025 });
  expect(message?.details?.pairs).toEqual(expect.arrayContaining([
...
it('keeps duplicate neutral output as a warning', () => {
  ...
  expect(result.diagnostics.messages).toEqual(expect.arrayContaining([
...
it('preserves both scale sources when merging theme diagnostics', () => {
  ...
  const messages = result.diagnostics.messages.filter((item) => item.code === 'LOW_ADJACENT_DIFFERENCE');
